...
## SECOND HANDOFF — same session day, second workhorse (runtime budget, nothing written yet)

Everything above still stands. This session re-verified the rmcp API directly from the
vendored crate source (`/tmp/rmcp-src/rmcp-3.1.2/` + `rmcp-macros-3.1.2/`) and adds:

- **schemars is re-exported by rmcp**: `src/lib.rs:40 pub use schemars;` → use
  `rmcp::schemars::JsonSchema` for tool-arg structs. **No extra dep needed — spec N2 holds.**
- `ServerInfo = InitializeResult` (model.rs:1085). `get_info()` is auto-generated by
  `#[tool_handler(name = "cast-tv-terminal", version = "0.1.0")]` →
  `Implementation::new(name, version)` (macros tool_handler.rs build_get_info) →
  E2E's `serverInfo.name == "cast-tv-terminal"` works with the name attribute.
- Tool methods: `async fn t(&self, Parameters(args): Parameters<Args>) -> Result<CallToolResult, McpError>`
  (or `Result<String, X>`). `Parameters<P>: FromContextPart` needs `P: DeserializeOwned`.
  `Result<T,E>` via `IntoCallToolResult` — **error branch auto-sets `is_error=Some(true)`**
  (tool.rs:130-144). `CallToolResult::success/error(Vec<ContentBlock>)` (model.rs:3882/3940),
  `ContentBlock::text(String)`.
- `#[tool_router(server_handler)]` requires an **inherent** impl block (errors on trait impl,
  tool_router.rs:91). Pattern: `impl McpServer { #[tool] async fn ... }` +
  `#[tool_router(server_handler)]` + separate `#[tool_handler(name=..., version=...)] impl
  ServerHandler for McpServer {}` (or put handler attrs directly on the router impl per
  macros docs; both expand the same).
- Serve: `handler.serve((tokio::io::stdin(), tokio::io::stdout())).await?.waiting().await`
  — `ServiceExt::serve` (service.rs:330), `stdio()` (transport/io.rs:4), `RunningService::waiting`
  (service.rs:1088). `ServerCapabilities::builder().enable_tools().build()`.
- Proc-macro feature flags confirmed in rmcp Cargo.toml: `macros` in default; `server`,
  `transport-io`, `schemars` are real feature names — the spec's feature list is valid.
- E2E handshake note: rmcp serves newline-delimited JSON; drive initialize →
  `{"jsonrpc":"2.0","id":1,"method":"initialize","params":{...}}` then read response; use
  `CARGO_BIN_EXE_mcp-server` in tests. `negotiate_protocol_version` echoes the client's
  version — send one of `ProtocolVersion::KNOWN_VERSIONS` values.
