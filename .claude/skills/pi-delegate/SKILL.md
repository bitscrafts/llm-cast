...
**Phases 2-6 of `sdd-orchestrate`.** Called by: `sdd-orchestrate`, after
`spec-author` has produced the contract.
Invokes the pi-side skills **`implementer`** (phase 2) and **`reviewer`**
(phase 5) through the harness.
Next phase: **`verify-and-commit`**.
...
`run` escalates automatically in two places: after `ORCH_MAX_REPAIR_ROUNDS`
failed gate rounds, and immediately if implement times out or changes nothing.
Escalate by hand for anything touching wire formats, migrations or concurrency.
