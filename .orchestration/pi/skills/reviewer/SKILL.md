...
**Phase 5.** Invoked by `pi-workhorse.sh review` after the quality gate
(`pi-workhorse.sh gate`, a bundled script — not a skill) has passed. You are reviewing what the `implementer` skill produced. Your verdict
feeds Claude's `verify-and-commit` skill, which decides whether to commit.
