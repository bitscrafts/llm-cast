...
required). AMENDED AGAIN 2026-08-16 after the implement pass: exit criterion 3
...
- **Cast sender (R6)**: the module MUST compile and its two tests MUST pass
  ...
  - `build_media_load_request(url: &str) -> serde_json::Value` is pure and
    unconditional. It must emit the Cast v2 media **LOAD** request — the wire
    field `"type": "LOAD"` plus the URL under `media` — which is what exit
    criterion 3 greps for. (Note: the string `"media/load"` is NOT part of the
    Cast v2 protocol; the message type is the `"type": "LOAD"` field. A grep
    for the bare `"media/load"` literal was comment-trivial and was replaced.)
